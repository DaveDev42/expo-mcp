...
function createLifecycleHandlers(managers) {
  return {
    ...
    async launch_expo(args2) {
      ...
      const phone = args2.auto_login?.phone || process.env.EXPO_TEST_PHONE;
      const password = args2.auto_login?.password || process.env.EXPO_TEST_PASSWORD;
      if (device && (args2.auto_login?.flow_file || phone && password)) {
        try {
          if (args2.auto_login?.flow_file) {
  ...
  return {
    ...
    async callTool(name, args2) {
      ...
      if (DEVICE_REQUIRED_TOOLS.includes(name)) {
        ...
        const deviceId = managers.expoManager.getDeviceId();
        enhancedArgs = { ...args2, device_id: deviceId };
      }
...
var McpServer = class {
  ...
  maestroProxy;
  toolFilterConfig;
  constructor(appDir2, toolFilter) {
