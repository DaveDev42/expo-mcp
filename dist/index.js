...
// src/managers/cdp-client.ts
import { randomUUID as randomUUID2 } from "crypto";
import { WebSocket as WebSocket2 } from "ws";

// src/managers/cdp-verify.ts
function filterHermesPages(pages, requireNativeReloads = true) {
...
function selectHermesRuntimePage(pages, pageId) {
...
function renderRemoteObject(arg) {
  ...
  if (arg.value !== void 0 && arg.value !== null) return String(arg.value);
...
function flattenConsoleArgs(params) {
  const args2 = params?.args ?? [];
  return args2.map(renderRemoteObject).join(" ").trim();
}
...
function judgeSignatures(entries, signatures) {
  return signatures.map((sig) => {
    const expect = sig.expect ?? "absent";
    const severity = sig.severity ?? "fail";
...
// src/managers/cdp-client.ts
async function fetchPageList(port) {
...
function captureDeviceConsole(opts) {
  return new Promise((resolve, reject) => {
    ...
    let nonceSent = false;
    const ws = new WebSocket2(opts.webSocketDebuggerUrl, { origin: opts.origin });
    let quietTimer = null;
...
var lifecycleToolSchemas = {
  ...
  inspect_console_pages: {
    ...
    description: "List the device debug pages reachable via the dev-server CDP inspector (GET /json/list). A device appears here ONLY if it loaded the Metro dev bundle (opened the inspector device WebSocket); a device that fell back to its embedded FILE bundle is structurally absent. Use this to confirm dev_bundle_attached before running verify_on_device. Requires: session must be running (call start_session first).",
    inputSchema: z.object({
      require_native_reloads: z.coerce.boolean().optional().default(true).describe(
        "When true (default), filter to non-synthetic Hermes runtime pages (capabilities.nativePageReloads===true) \u2014 pages that prove a dev-server bundle is loaded. When false, return every page (incl. the synthetic id=-1 page) with isHermesRuntime/isSynthetic flags."
      )
    })
  },
  verify_on_device: {
    name: "verify_on_device",
    description: "Verify console signatures on a live device over CDP, with a structurally non-fakeable liveness gate. PASS requires: (1) a non-synthetic Hermes runtime page exists (device is on the dev bundle, not a file-bundle fallback), (2) the reload reached a live client, (3) a fresh post-reload execution context + nonce round-trip proves the bundle re-ran, (4) the capture window observed >=1 entry, AND (5) every fail-severity signature is satisfied. Any precondition failure returns verdict:FAIL with a distinct reason (never a silent 0-match clean pass). Requires: session must be running (call start_session first).",
    inputSchema: z.object({
      reload: z.coerce.boolean().optional().default(true).describe("Reload before capture so the fix re-executes from a clean module graph. Required true for a real regression check."),
      settle_quiet_ms: z.coerce.number().optional().default(1500).describe("Stop capturing after this many ms with zero new console events (settle-based window, not a fixed sleep)."),
      max_capture_ms: z.coerce.number().optional().default(8e3).describe("Hard ceiling on the capture window in ms."),
      require_dev_bundle: z.coerce.boolean().optional().default(true).describe(
        "Hard gate. When true (default), all liveness preconditions must pass or the whole verification FAILS \u2014 making a false-clean impossible. Exposed only so a human can intentionally observe a non-dev-bundle device."
      ),
      page_id: z.string().optional().describe("Target page id from inspect_console_pages. Omit to auto-pick the single non-synthetic Hermes page; required only when >1 is attached."),
      signatures: z.array(
        z.object({
          ...
          expect: z.enum(["absent", "present"]).optional().default("absent").describe("absent = FAIL on any match (regression guard, the common case); present = FAIL if no match (positive assertion)."),
          severity: z.enum(["fail", "warn"]).optional().default("fail").describe("warn = reported in results but does not block PASS (for expected residual noise)."),
          channels: z.array(z.enum(["console", "log"])).optional().describe("Restrict to console (Runtime.consoleAPICalled) and/or log (Log.entryAdded). Default both.")
        })
      ).min(1).describe("Console signatures to judge. PASS = all liveness preconditions met AND every fail-severity signature satisfied.")
    })
  },
  press_key: {
    name: "press_key",
...
function createLifecycleHandlers(managers) {
  ...
  return {
    ...
    async inspect_console_pages(args2) {
      if (managers.expoManager.getStatus() !== "running") {
        throw new Error("Session is not running. Call start_session first.");
      }
      ...
      try {
        ...
        pages = filterHermesPages(raw, args2.require_native_reloads);
      } catch (err) {
        return {
          content: [
            {
    ...
    async verify_on_device(args2) {
      ...
      const target = selectHermesRuntimePage(rawPages, args2.page_id);
      const devBundleAttached = filterHermesPages(rawPages, true).length > 0;
      if (args2.require_dev_bundle && !target) {
        const candidates = filterHermesPages(rawPages, true);
        if (candidates.length > 1) {
          return fail("capture_failed", {
            dev_bundle_attached: true,
            error: `Multiple dev-bundle pages attached; pass page_id. Candidates: ${candidates.map((c) => c.id).join(", ")}`
          });
        }
      ...
      try {
        capture = await captureDeviceConsole({
          ...
          origin,
          settleQuietMs: args2.settle_quiet_ms,
          maxCaptureMs: args2.max_capture_ms,
          onSubscribed: args2.reload ? async () => {
            await managers.expoManager.reload();
          } : void 0
        });
      } catch (err) {
      ...
      const fileFallbackSeen = stderrLogs.some(
        (l) => /No apps connected\.\s*Sending\s*"reload"/i.test(l.message)
      );
      const reloadDelivered = args2.reload ? !fileFallbackSeen : true;
      const results = judgeSignatures(capture.entries, args2.signatures);
      const { verdict, reason } = decideVerdict({
        devBundleAttached: true,
        reloadRequested: args2.reload,
        reloadDelivered,
...
var McpServer = class {
  ...
  zodToJsonSchema(zodSchema) {
    ...
    if (hasDefault && defaultValue !== void 0) out.default = defaultValue;
    switch (typeName) {
      case "ZodString":
        out.type = "string";
        break;
